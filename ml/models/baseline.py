"""RingGuard AI — Baseline XGBoost Model (Model A).

Stage 6: Transaction-Only Baseline XGBoost.
Trains a binary XGBoost classifier using Transaction + Behavior features only (37 features).
Explicitly excludes all graph/network features, targets, and raw entity IDs.
"""
...
class BaselineXGBoostModel:
    """Manages data preparation, chronological splitting, training, and inference for Model A."""
    ...
    def load_dataset(self) -> Tuple[pd.DataFrame, pd.Series, pd.DataFrame]:
        """Load Model A features and target metadata, enforcing strict alignment.
        
        Returns:
            (X, y, metadata) sorted chronologically.
        """
        features_path = self.data_dir / "model_a_features.csv"
        meta_path = self.data_dir / "target_metadata.csv"

        if not features_path.exists():
            raise FileNotFoundError(f"Model A features file not found: {features_path}")
        ...
        # Integrity assertion: exact 2,000 rows, matching indices
        ...
        if not (X.index == meta.index).all():
            raise ValueError("Row index mismatch between Model A features and target metadata.")

        # Ensure no graph features exist in Model A
        graph_cols = [c for c in X.columns if c.startswith("g_")]
        if graph_cols:
            raise ValueError(f"Prohibited graph features found in Model A: {graph_cols}")
        ...
        if leakage_cols:
            raise ValueError(f"Target leakage columns found in Model A: {leakage_cols}")

        # Sort chronologically by timestamp, then transaction_id
    ...
    def train(self, X_train: pd.DataFrame, y_train: pd.Series) -> None:
        ...
        # Calculate scale_pos_weight strictly on training data
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        ...
        # Column 1 corresponds to positive class
        return self.model.predict_proba(X)[:, 1]

    def get_feature_importances(self) -> pd.DataFrame:
        """Return feature importances ranked by gain/weight."""
        if self.model is None:
            raise RuntimeError("Model has not been trained yet.")
    ...
    def save_artifacts(
        self,
        model_path: str = "models/ringguard_baseline_xgb_v1.joblib",
        metadata_path: str = "models/ringguard_baseline_xgb_v1_metadata.json",
    ...
    @classmethod
    def load(cls, model_path: str = "models/ringguard_baseline_xgb_v1.joblib") -> "BaselineXGBoostModel":
